...
class Solver:
    ...
    def load_input(self, data: Data):
        ...
        self.allocated = self.solver.IntVar(-np.ma.size(self.data.old_allocations_matrix), 0, "allocated")
        self.deallocated = self.solver.IntVar(-np.ma.size(self.data.old_allocations_matrix), 0, "deallocated")
        # self.free_delete = self.solver.NumVar(0, 1, "free delete")
        ...
        # Find a solution with good delays routing
        self.solver.Add(
            self.solver.Sum([
        ...
        # Compute the number of migrations
        self.solver.Add(self.allocated <= 0)
        self.solver.Add(self.solver.Sum(
            [data.old_allocations_matrix[f, i]
             for f, i in itertools.product(range(len(data.functions)), range(len(data.nodes)))]) -
                        self.solver.Sum([self.c[f, i]
                                         for f, i in itertools.product(range(len(data.functions)),
                                                                       range(len(data.nodes)))]) >= self.allocated)
        self.solver.Add(self.deallocated <= 0)
        self.solver.Add(self.solver.Sum(
            [self.c[f, i]
             for f, i in itertools.product(range(len(data.functions)), range(len(data.nodes)))]) -
                        self.solver.Sum([data.old_allocations_matrix[f, i]
                                         for f, i in itertools.product(range(len(data.functions)),
                                                                       range(len(data.nodes)))]) >= self.deallocated)
        ...
        # self.solver.Add(self.free_delete <= 1)
    ...
    def solve(self):
        ...
        w = np.ma.size(self.data.old_allocations_matrix)
        for f in range(len(self.data.functions)):
            for j in range(len(self.data.nodes)):
                self.objective.SetCoefficient(self.moved_from[f, j], w)
                self.objective.SetCoefficient(self.moved_to[f, j], w)
        self.objective.SetCoefficient(self.allocated, w - 1)
        self.objective.SetCoefficient(self.deallocated, w + 1)
